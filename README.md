...
2. Copy `.env.example` to `.env.local` and fill in real values (Supabase project URL/keys, Stripe test keys, Resend key, `ADMIN_INBOX_EMAIL` for contact-form and new-subscriber notifications). `NEXT_PUBLIC_SITE_URL` should stay `http://localhost:3001` for local dev.
