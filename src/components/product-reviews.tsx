...
export function ProductReviews({
  ...
  return (
    <section className="flex flex-col gap-6 border-t border-zinc-700 pt-8">
      <div className="flex items-center gap-3">
        ...
        {count > 0 ? (
          ...
          <span className="text-sm text-zinc-300">No reviews yet</span>
        )}
      </div>
      ...
      <ul className="flex flex-col gap-4">
        {existingReview && (
          <li className="flex flex-col gap-1 rounded border border-zinc-700 p-4">
            ...
            {existingReview.body && <p className="text-sm text-zinc-200">{existingReview.body}</p>}
          </li>
        )}
        {otherReviews.map((review) => (
          <li key={review.id} className="flex flex-col gap-1 rounded border border-zinc-700 p-4">
            <div className="flex items-center gap-2">
              <StarRating rating={review.rating} size="text-sm" />
              <span className="text-sm font-medium">{review.reviewerName}</span>
            </div>
            {review.body && <p className="text-sm text-zinc-200">{review.body}</p>}
          </li>
        ))}
      </ul>
    </section>
